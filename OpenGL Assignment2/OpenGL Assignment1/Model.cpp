...
Model::Model(GLchar* path, std::string _name, bool _centered)	{
	name = _name;
	centered = _centered;
	Mesh::unique_ID = 0;
	mID = unique_ID;
...
void Model::Translate(glm::vec3 change) {

	glm::mat4 trans = glm::translate(glm::mat4(), change);

	for (int i = 0; i < meshes.size(); i++) {
		meshes[i].ModelMat = meshes[i].ModelMat * trans;
	}
}
